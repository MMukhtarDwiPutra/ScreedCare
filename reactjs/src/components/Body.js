        ...
        <Row xs="auto" gy="5" className="justify-content-center">
          {items.map((item) => (
            <Col className="col-md-4 col-offset-5">
              <Cardbody image={item.foto.includes("http") ? item.foto : process.env.REACT_APP_SERVICE_URL+"/static/img/"+item.foto} namabarang={item.nama_barang} hargabarang={item.harga_str} link={item.link_shopee}/>
            </Col>
          ))}
        </Row>
      </div>
